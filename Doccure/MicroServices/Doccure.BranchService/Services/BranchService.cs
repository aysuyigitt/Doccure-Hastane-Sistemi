...
namespace Doccure.BranchService.Services
{
    public class BranchService : IBranchService
    {
        ...
        public BranchService(IDatabaseSettings settings, IMapper mapper)
        {
        ...
        public async Task<GetBranchIdDto> GetByIdAsync(string id)
        {
            var value = await _branchCollection.Find(x => x.BranchId == id).FirstOrDefaultAsync();
            return _mapper.Map<GetBranchIdDto>(value);
        }
